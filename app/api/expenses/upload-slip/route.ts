...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `นี่คือรูปสลิปโอนเงินจากแอปธนาคารไทย ช่วยอ่านแล้วตอบกลับเป็น JSON เท่านั้น ห้ามมีข้อความอื่นเลย

สำคัญ: ให้มองทั้งภาพอย่างละเอียด รวมถึงตัวหนังสือเล็กๆ ที่อยู่ส่วนล่างสุดของภาพด้วย ไม่ว่าตัวหนังสือนั้นจะมีป้ายกำกับชัดเจนว่า "หมายเหตุ" หรือ "บันทึกช่วยจำ" หรือไม่มีป้ายกำกับใดๆ เลยก็ตาม ข้อความอิสระที่ผู้โอนพิมพ์เพิ่มเติมไว้ (ไม่ใช่ชื่อธนาคาร ไม่ใช่เลขบัญชี ไม่ใช่เลขอ้างอิงธุรกรรม) ให้ถือว่าเป็นหมายเหตุทั้งหมด แม้จะอยู่แค่บรรทัดเดียวสั้นๆ ท้ายภาพก็ตาม
...
ตอบกลับในรูปแบบ:
{
  ...
  "note": "ข้อความอิสระที่ผู้โอนพิมพ์เพิ่มเติมไว้ ตามที่อธิบายด้านบน ถ้าหาไม่เจอจริงๆ ทั่วทั้งภาพให้เป็น null",
  "suggestedCategory": "เลือก 1 หมวดจากลิสต์นี้ที่ใกล้เคียงกับหมายเหตุที่สุด: ${CATEGORIES.join(", ")} — ไม่ต้องให้หมายเหตุตรงกับชื่อหมวดเป๊ะๆ ให้ตีความแบบยืดหยุ่น เช่น 'ซื้อผัก', 'ตลาดนัด', 'ของสด' ให้เดาเป็นวัตถุดิบ, 'เติมแก๊สหุงต้ม' ให้เดาเป็นค่าแก๊ส, 'ถุง กล่องข้าว' ให้เดาเป็นค่าบรรจุภัณฑ์, ข้อความที่สื่อถึงเรื่องส่วนตัวชัดเจน (เช่น 'ค่ากับข้าวบ้าน', 'ซื้อของใช้ส่วนตัว') ให้เดาเป็นส่วนตัว เป็นต้น ถ้าไม่มีหมายเหตุเลยหรือไม่มีทางเดาได้เลยจริงๆ ให้เป็น null"
}`;
